...
contract Exchange {
    ...
    */
    ...
    //@return 1: 是否交易成功
    */
    function transactFull(address receiver, string details, bytes32[] tkids, bytes32[] splittkids, bytes32[7][] hashs, address[2][] addrs, uint[9][] ints) public returns (bool) {
        bool flag = true;
        if (msg.sender != 0x00 && msg.sender != block.coinbase) {
            for (uint i = 0; i < tkids.length; i++) {
                flag = transToken(receiver, tkids[i], ints[0][5]);
            }
            if (splittkids.length > 0) {
                for (uint j = 0; j < splittkids.length; j++){
                    flag = setToken(splittkids[j], "0x11", ints[0][6], 6);
                }
