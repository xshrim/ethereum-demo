...
contract Exchange {
    ...
    */
    ...
    event SetReceipt (address sender, address indexed payer, address indexed payee, string detail, uint timestamp);
    ...
    */
    ...
    //@return 1: 地址的类型
    */
    function setCustAddr(address addr, string no, string hash, string info, uint64 level, uint64 status) public onlyBy returns (bool) {
    ...
    //@return 1: 是否设置成功
    */
    function setCustStatusByAddr(address addr, uint64 newstatus) public onlyBy returns (bool) {
    ...
    //@return 1: 是否设置成功
    */
    function setCustStatusByNo(string no, uint64 newstatus) public onlyBy returns (bool) {
        return setCustStatusByAddr(custaddrs[no], newstatus);
    }

    /* 设置客户状态By索引
    //@return 1: 是否设置成功
    */
    function setCustStatusByIdx(uint64 idx, uint64 newstatus) public onlyBy returns (bool) {
        return setCustStatusByAddr(addridxs[idx], newstatus);
    }
    ...
    */
    ...
    //@return 1: 是否记录成功
    */
    function setReceipt(address payer, address payee, string details, uint64 timestamp) public returns (bool) {
        emit PayReceipt(msg.sender, payer, payee, details, timestamp);
        return true;
    }
    ...
    */
    ...
    //@return 1: 是否分配成功
    */
    function assignToken(address receiver, string details, bytes16[9] hashs, address[4] addrs, uint64[10] ints) public onlyBy returns (bool) {
        // if (msg.sender == block.coinbase && issueToken(receiver, hashs, addrs, ints)) {
        if (issueToken(receiver, hashs, addrs, ints)) {
            emit PayReceipt(msg.sender, msg.sender, receiver, details, ints[3]);
            return true;
        } else {
            revert();
        }
    ...
    */
    ...
    //@return 1: 是否分配成功
    */
    function conferToken(address receiver, string details, bytes16[9] hashs, address[4] addrs, uint64[10] ints) public onlyBy returns (bool) {
        bool flag = false;
        if (assignToken(msg.sender, details, hashs, addrs, ints) && ledgers[hashs[0]] == msg.sender) {
            if (transToken(msg.sender, receiver, hashs[0], ints[6], 0)) {
                emit PayReceipt(msg.sender, msg.sender, receiver, details, ints[6]);
                flag = true;
            }
        }
        if (flag == false) {
    ...
    //@return 1: 是否分配成功
    */
    function conferMyToken(address receiver, string details, bytes16 tkid, uint64 ctimestamp) public onlyBy returns (bool) {
        bool flag = false;
    ...
    */
    function transferToken(address receiver, string details, bytes16[] tkids, uint64 ctimestamp) public returns (bool) {
        ...
        for (uint i = 0; i < tkids.length && flag == true; i++) {
            flag = payMyToken(receiver, tkids[i], ctimestamp);
        }
        emit PayReceipt(msg.sender, msg.sender, receiver, details, ctimestamp);
        return flag;
    }
    ...
    //@return 1: 是否交易成功
    */
    function transact(address receiver, string details, bytes16[] tkids, bytes16[] splittkids, bytes16[9][] hashs, address[4][] addrs, uint64[10][] ints) public returns (bool) {
        bool flag = true;
        for (uint i = 0; i < tkids.length && flag == true; i++) {
            flag = payMyToken(receiver, tkids[i], ints[0][6]);
        }
        if (splittkids.length > 0 && flag == true) {
            ...
            for (uint k = 0; k < hashs.length && flag == true; k++) {
                if (k % 2 == 0) {
                    flag = issueToken(msg.sender, [hashs[k][0], hashs[k][1], hashs[k][2], hashs[k][3], hashs[k][4], hashs[k][5], hashs[k][6], hashs[k][7], hashs[k][7]], [addrs[k][0], addrs[k][1], addrs[k][2], addrs[k][3]], [ints[k][0], ints[k][1], ints[k][2], ints[k][3], ints[k][4], ints[k][5], ints[k][6], ints[k][7], ints[k][8], ints[k][9]]);
                } else {
                    flag = issueToken(receiver, [hashs[k][0], hashs[k][1], hashs[k][2], hashs[k][3], hashs[k][4], hashs[k][5], hashs[k][6], hashs[k][7], hashs[k][7]], [addrs[k][0], addrs[k][1], addrs[k][2], addrs[k][3]], [ints[k][0], ints[k][1], ints[k][2], ints[k][3], ints[k][4], ints[k][5], ints[k][6], ints[k][7], ints[k][8], ints[k][9]]);
                }
            }
        }
    ...
    //@return 1: 是否交易成功
    */
    function transactAll(address[] parts, string details, bytes16[] atkids, bytes16[] btkids, bytes16[] asplittkids, bytes16[] bsplittkids, bytes16[9][] hashs, address[4][] addrs, uint64[10][] ints) public onlyBy returns (bool) {
        bool flag = true;
        for (uint i = 0; i < atkids.length && flag == true; i++) {
            flag = payToken(parts[0], parts[1], atkids[i], ints[0][6]);
        }
        for (i = 0; i < btkids.length && flag == true; i++) {
            flag = payToken(parts[1], parts[0], btkids[i], ints[0][6]);
        }
        if (flag == true && (asplittkids.length > 0 || bsplittkids.length > 0)) {
            for (i = 0; i < asplittkids.length && flag == true; i++) {
                flag = chTokenStatus(asplittkids[i], "0x11", ints[0][7], 6);
            }
            for (i = 0; i < bsplittkids.length && flag == true; i++) {
                flag = chTokenStatus(bsplittkids[i], "0x11", ints[0][7], 6);
            }
    ...
    */
    ...
    //@return 1: 是否设置成功
    */
    function setMyTokenStatus(string details, bytes16[] tkids, bytes16 statuscode, uint64 stimestamp, uint64 status) public returns (bool) {
        ...
        emit SetReceipt(msg.sender, msg.sender, msg.sender, details, stimestamp);
    }
    ...
    //@return 1: 是否设置成功
    */
    function setTokenStatus(string details, bytes16[] tkids, bytes16 statuscode, uint64 stimestamp, uint64 status) public onlyBy returns (bool) {
        for (uint i = 0; i < tkids.length; i++) {
            if (msg.sender == ledgers[tkids[i]]) {
                if (chTokenStatus(tkids[i], statuscode, stimestamp, status) != true) {
                    revert();
