...
contract RS {
    ...
    */
    struct Item {
        bytes16 id;             // 资源id
        bytes16 pid;            // 资源父id(0x00表示资源本身是原始资源)
        bytes16 sid;            // 资源未加密源文件id(0x00表示资源本身是未加密源文件)
    ...
    */
    event WatchDog (address addr, bytes16 themsg, string thimsg, uint indexed res);
    ...
    */
    event TXReceipt (bytes16 indexed sn, address sender, bytes16 indexed userid, bytes16 indexed itemid, bytes16 permid, bytes32 action, string details, uint64 duration, uint64 timestamp);
    //event TXReceipt (bytes16 indexed sn, address sender, bytes16 indexed userid, bytes16 indexed itemid, bytes16 permid, bytes16 rhash, bytes32 action, string details, uint64 duration, uint64 timestamp);
    ...
    constructor() public payable{
        ...
        users["0"] = User(msg.sender, "0", "admin", "owner", 1, uint64(block.timestamp), 1);
    ...
    */
    ...
    //@return 0: 是否设置成功
    */
    function setUser(address addr, bytes16 id, bytes16 utype, bytes16 hash, uint64 level, uint64 status, uint64 timestamp) internal returns (bool) {
        uidxids[usernum++] = id;
        uidaddrs[id] = addr;
        uaddrids[addr] = id;
        users[id] = User(addr, id, utype, hash, level, timestamp, status);
        return true;
    }
    ...
    //@return 0: 是否设置成功
    */
    function addUser(bytes16 sn, string details, address addr, bytes16 id, bytes16 utype, bytes16 uhash, uint64 level, uint64 status) public payable onlyAdmin returns (bool) {
        if (setUser(addr, id, utype, uhash, level, status, uint64(block.timestamp))) {
            addr.transfer(1000000000000000000000);
            emit TXReceipt(sn, msg.sender, id, 0x00, 0x00, "addUser", details, 0, uint64(block.timestamp));
    ...
    */
    function setUserStatusByAddr(bytes16 sn, string details, address addr, uint64 newstatus) public onlyAdmin returns (bool) {
        if (setUserStatus(uaddrids[addr], newstatus)) {
            emit TXReceipt(sn, msg.sender, uaddrids[addr], 0x00, 0x00, "setUserStatusByAddr", details, 0, uint64(block.timestamp));
    ...
    */
    function setUserStatusById(bytes16 sn, string details, bytes16 id, uint64 newstatus) public onlyAdmin returns (bool) {
        if (setUserStatus(id, newstatus)) {
            emit TXReceipt(sn, msg.sender, id, 0x00, 0x00, "setUserStatusById", details, 0, uint64(block.timestamp));
    ...
    */
    function setUserStatusByIdx(bytes16 sn, string details, uint64 idx, uint64 newstatus) public onlyAdmin returns (bool) {
        if (setUserStatus(uidxids[idx], newstatus)) {
            emit TXReceipt(sn, msg.sender, uidxids[idx], 0x00, 0x00, "setUserStatusByIdx", details, 0, uint64(block.timestamp));
    ...
    //@return 0: 是否设置成功
    */
    function setUserHash(bytes16 id, bytes16 newhash) internal returns (bool) {
        if (id == 0x00) {
            revert();
        } else {
            User storage user = users[id];
    ...
    //@return 0: 是否设置成功
    */
    function setUserHashByAddr(bytes16 sn, string details, address addr, bytes16 newhash) public onlyAdmin returns (bool) {
        if (setUserHash(uaddrids[addr], newhash)) {
            emit TXReceipt(sn, msg.sender, uaddrids[addr], 0x00, 0x00, "setUserHashByAddr", details, 0, uint64(block.timestamp));
    ...
    //@return 0: 是否设置成功
    */
    function setUserHashById(bytes16 sn, string details, bytes16 id, bytes16 newhash) public onlyAdmin returns (bool) {
        if (setUserHash(id, newhash)) {
            emit TXReceipt(sn, msg.sender, id, 0x00, 0x00, "setUserHashById", details, 0, uint64(block.timestamp));
            //setLog([sn, uaddrids[msg.sender], 0x00, 0x00], 0x00, "setUserHashById", details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    //@return 0: 是否设置成功
    */
    function setUserHashByIdx(bytes16 sn, string details, uint64 idx, bytes16 newhash) public onlyAdmin returns (bool) {
        if (setUserHash(uidxids[idx], newhash)) {
            emit TXReceipt(sn, msg.sender, uidxids[idx], 0x00, 0x00, "setUserHashByIdx", details, 0, uint64(block.timestamp));
            //setLog([sn, uaddrids[msg.sender], 0x00, 0x00], 0x00, "setUserHashByIdx", details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    ...
    //@return 6: 用户状态
    */
    function getUserById(bytes16 id) public view returns (address, bytes16, bytes16, bytes16, uint64, uint64, uint64) {
        if (id == 0x00) {
            return (0x00, bytes16(0x00), 0x00, 0x00, uint64(0), 0, 0);
        } else {
            User memory user = users[id];
            return (user.addr, user.id, user.utype, user.uhash, user.level, user.timestamp, user.status);
        }
    }
    ...
    //@return 6: 用户状态
    */
    function getUserByAddr(address addr) public view returns (address, bytes16, bytes16, bytes16, uint64, uint64, uint64) {
    ...
    //@return 6: 用户状态
    */
    function getUserByIdx(uint64 idx) public view returns (address, bytes16, bytes16, bytes16, uint64, uint64, uint64) {
    ...
    */
    ...
    //@return 0: 是否设置成功
    */
    function setItem(bytes16[4] ids, bytes16 itype, bytes16 ihash, bytes16 xhash, uint64 level, uint64 timestamp, uint64 status) internal returns (bool) {
    ...
    */
    function addItem(bytes16 sn, string details, bytes16[] ids, bytes16 itype, bytes16 ihash, bytes16 xhash, uint64 level, uint64 status) public onlyAdmin returns (bool) {
        ...
        if (setItem([ids[0], ids[1], ids[2], senderid], itype, ihash, xhash, level, uint64(block.timestamp), status)) {
            emit TXReceipt(sn, msg.sender, senderid, ids[0], 0x00, "addItem", details, 0, uint64(block.timestamp));
    ...
    //@return 0: 是否设置成功
    */
    function uplItem(bytes16 sn, string details, bytes16[3] ids, bytes16 itype, bytes16 ihash, bytes16 xhash, uint64 level, uint64 status) public onlyValid returns (bool) {
        bytes16 senderid = uaddrids[msg.sender];
        if (setItem([ids[0], ids[1], ids[2], senderid], itype, ihash, xhash, level, uint64(block.timestamp), status)) {
            emit TXReceipt(sn, msg.sender, senderid, ids[0], 0x00, "uplItem", details, 0, uint64(block.timestamp));
    ...
    */
    function setItemStatusById(bytes16 sn, string details, bytes16 id, uint64 newstatus) public onlyValid returns (bool) {
        if (setItemStatus(id, newstatus)) {
            emit TXReceipt(sn, msg.sender, 0x00, id, 0x00, "setItemStatusById", details, 0, uint64(block.timestamp));
    ...
    */
    function setItemStatusByIdx(bytes16 sn, string details, uint64 idx, uint64 newstatus) public onlyValid returns (bool) {
        ...
        if (setItemStatus(id, newstatus)) {
            emit TXReceipt(sn, msg.sender, 0x00, id, 0x00, "setItemStatusByIdx", details, 0, uint64(block.timestamp));
    ...
    */
    ...
    //@return 6: 资源状态
    */
    function getItemById(bytes16 id) public view returns (bytes16[4], bytes16, bytes16, bytes16, uint64, uint64, uint64) {
        if (id == 0x00) {
            return ([bytes16(0x00), 0x00, 0x00, 0x00], bytes16(0x00), 0x00, "", uint64(0), 0, 0);
        } else {
            Item memory item = items[id];
            return ([item.id, item.pid, item.sid, item.userid], item.itype, item.ihash, item.xhash, item.level, item.timestamp, item.status);
        }
    }
    ...
    //@return 6: 资源状态
    */
    function getItemByIdx(uint64 idx) public view returns (bytes16[4], bytes16, bytes16, bytes16, uint64, uint64, uint64) {
    ...
    */
    function addPerm(bytes16 sn, string details, bytes16[4] ids, bytes16 device, uint64[3] ptype, uint64[2] ptimestamp, uint64[2] ptime, uint64[2] ptimes, uint64 status) public onlyValid returns (bool) {
        if (setPerm(ids, device, ptype, ptimestamp, ptime, ptimes, status)) {
            ...
            emit TXReceipt(sn, msg.sender, ids[1], ids[2], ids[0], "addPerm", details, 0, uint64(block.timestamp));
    ...
    */
    function setPermTimeById(bytes16 sn, string details, bytes16 id, uint64 utime) public onlyValid returns (bool) {
        if (setPermTime(id, utime)) {
            ...
            emit TXReceipt(sn, msg.sender, perm.userid, perm.itemid, id, "setPermTimeById", details, 0, uint64(block.timestamp));
    ...
    */
    function setPermTimeByIdx(bytes16 sn, string details, uint64 idx, uint64 utime) public onlyValid returns (bool) {
        ...
        if (setPermTime(id, utime)) {
            Perm memory perm = perms[id];
            emit TXReceipt(sn, msg.sender, perm.userid, perm.itemid, id, "setPermTimeByIdx", details, 0, uint64(block.timestamp));
    ...
    */
    function setPermStatusById(bytes16 sn, string details, bytes16 id, uint64 newstatus) public onlyValid returns (bool) {
        if (setPermStatus(id, newstatus)) {
            Perm memory perm = perms[id];
            emit TXReceipt(sn, msg.sender, perm.userid, perm.itemid, id, "setPermStatusById", details, 0, uint64(block.timestamp));
    ...
    */
    function setPermStatusByIdx(bytes16 sn, string details, uint64 idx, uint64 newstatus) public onlyValid returns (bool) {
        ...
        if (setPermStatus(id, newstatus)) {
            Perm memory perm = perms[id];
            emit TXReceipt(sn, msg.sender, perm.userid, perm.itemid, id, "setPermStatusByIdx", details, 0, uint64(block.timestamp));
    ...
    */
    ...
    //@return 0: 是否设置成功
    */
    function setLog(bytes16[4] ids, bytes32 action, string desc, uint64 duration, uint64 timestamp) internal returns (bool) {
    ...
    //@return 0: 是否设置成功
    */
    function addLog(bytes16 sn, string details, bytes16[4] ids, bytes32 action, string desc, uint64 duration, uint64 mode) public onlyAdmin returns (bool) {
        if (setLog(ids, action, desc, duration, uint64(block.timestamp))) {
            ...
            emit TXReceipt(sn, msg.sender, ids[1], ids[2], ids[0], "addLog", details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    ...
    //@return 4: 日志生成时间戳
    */
    function getLogById(bytes16 id) public view returns (bytes16[4], bytes32, string, uint64, uint64) {
        if (id == 0x00) {
            return ([bytes16(0x00), 0x00, 0x00, 0x00], bytes32(0x00), "", uint64(0), 0);
        } else {
            Log memory log = logs[id];
            return ([log.id, log.userid, log.itemid, log.permid], log.action, log.desc, log.duration, log.timestamp);
        }
    }
    ...
    //@return 4: 日志生成时间戳
    */
    function getLogByIdx(uint64 idx) public view returns (bytes16[4], bytes32, string, uint64, uint64) {
