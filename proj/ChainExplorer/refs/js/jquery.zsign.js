...
$.fn.zSign = function (options) {
    ...
    var range = {
        minX: _s.offsetx,
        minY: _s.offsety,
        maxX: _parent.width() - _s.width - _s.offsetx - 18, //扣去2个padding=8px以及2个边框1px
        maxY: _parent.height() - _s.height - _s.offsety - 18
    };
    if (!_s.signit) {
        var _btnPanel = $("<div class='panel'><button class='btn add' >盖 章</button><button class='btn cancel'>关 闭</button></div>").appendTo(_parent);
        var _html = "<div class='sign' style='height:" + _s.height + "px;width:" + _s.width + "px;top:" + _s.offsety + "px;left:" + _s.offsetx + "px'><img src='" + _s.img + "' draggable='false'/><button class='btn ok' >确定</button><button class='btn del' >删除</button></div>";
        ...
        var _html = "<div class='sign ok' style='height:" + _s.height + "px;width:" + _s.width + "px;top:" + _s.offsety + "px;left:" + _s.offsetx + "px'><img src='" + _s.img + "' draggable='false'/></div>";
