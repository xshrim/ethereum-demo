...
contract RS {
    ...
    */
    ...
    //@return 0: 是否设置成功
    */
    function setUser(address addr, bytes32 id, bytes32 utype, bytes32 name, bytes32 hash, uint64 level, uint64 status, uint64 timestamp) internal returns (bool) {
        uidxids[usernum++] = id;
        uidaddrs[id] = addr;
        uaddrids[addr] = id;
    ...
    //@return 0: 是否设置成功
    */
    function addUser(bytes32 sn, string details, address addr, bytes32 id, bytes32 utype, bytes32 name, bytes32 uhash, uint64 level, uint64 status) public payable onlyAdmin returns (bool) {
        if (setUser(addr, id, utype, name, uhash, level, status, uint64(block.timestamp))) {
            ...
            setLog([sn, uaddrids[msg.sender], 0x00, 0x00], "addUser", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    function setUserStatusByAddr(bytes32 sn, string details, address addr, uint64 newstatus) public onlyAdmin returns (bool) {
        if (setUserStatus(uaddrids[addr], newstatus)) {
            ...
            setLog([sn, uaddrids[msg.sender], 0x00, 0x00], "setUserStatusByAddr", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    function setUserStatusById(bytes32 sn, string details, bytes32 id, uint64 newstatus) public onlyAdmin returns (bool) {
        if (setUserStatus(id, newstatus)) {
            ...
            setLog([sn, uaddrids[msg.sender], 0x00, 0x00], "setUserStatusById", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    function setUserStatusByIdx(bytes32 sn, string details, uint64 idx, uint64 newstatus) public onlyAdmin returns (bool) {
        if (setUserStatus(uidxids[idx], newstatus)) {
            ...
            setLog([sn, uaddrids[msg.sender], 0x00, 0x00], "setUserStatusByIdx", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    ...
    //@return 0: 是否设置成功
    */
    function setItem(bytes32[2] ids, bytes32 itype, bytes32 ihash, bytes32 xhash, uint64 level, uint64 timestamp, uint64 status) internal returns (bool) {
    ...
    //@return 0: 是否设置成功
    */
    function addItem(bytes32 sn, string details, bytes32[] ids, bytes32 itype, bytes32 ihash, bytes32 xhash, uint64 level, uint64 status) public onlyAdmin returns (bool) {
        ...
        if (setItem([ids[0], senderid], itype, ihash, xhash, level, uint64(block.timestamp), status)) {
            ...
            setLog([sn, senderid, ids[0], 0x00], "addItem", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    //@return 0: 是否设置成功
    */
    function uplItem(bytes32 sn, string details, bytes32 itemid, bytes32 itype, bytes32 ihash, bytes32 xhash, uint64 level, uint64 status) public onlyValid returns (bool) {
        bytes32 senderid = uaddrids[msg.sender];
        if (setItem([itemid, senderid], itype, ihash, xhash, level, uint64(block.timestamp), status)) {
            ...
            setLog([sn, senderid, 0x00, 0x00], "uplItem", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    function setItemStatusById(bytes32 sn, string details, bytes32 id, uint64 newstatus) public onlyValid returns (bool) {
        if (setItemStatus(id, newstatus)) {
            ...
            setLog([sn, uaddrids[msg.sender], id, 0x00], "setItemStatusById", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    function setItemStatusByIdx(bytes32 sn, string details, uint64 idx, uint64 newstatus) public onlyValid returns (bool) {
        ...
        if (setItemStatus(id, newstatus)) {
            ...
            setLog([sn, uaddrids[msg.sender], id, 0x00], "setItemStatusByIdx", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    ...
    //@return 6: 资源状态
    */
    function getItemById(bytes32 id) public view returns (bytes32[2], bytes32, bytes32, bytes32, uint64, uint64, uint64) {
        if (id == 0x00) {
            return ([bytes32(0x00), 0x00], bytes32(0x00), 0x00, "", uint64(0), 0, 0);
        } else {
    ...
    //@return 6: 资源状态
    */
    function getItemByIdx(uint64 idx) public view returns (bytes32[2], bytes32, bytes32, bytes32, uint64, uint64, uint64) {
    ...
    */
    function addPerm(bytes32 sn, string details, bytes32[4] ids, bytes32 device, uint64[3] ptype, uint64[2] ptimestamp, uint64[2] ptime, uint64[2] ptimes, uint64 status) public onlyValid returns (bool) {
        if (setPerm(ids, device, ptype, ptimestamp, ptime, ptimes, status)) {
            ...
            setLog([sn, uaddrids[msg.sender], ids[2], ids[1]], "addPerm", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    function setPermTimeById(bytes32 sn, string details, bytes32 id, uint64 utime) public onlyValid returns (bool) {
        if (setPermTime(id, utime)) {
            ...
            setLog([sn, uaddrids[msg.sender], perm.itemid, id], "setPermTimeById", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    function setPermTimeByIdx(bytes32 sn, string details, uint64 idx, uint64 utime) public onlyValid returns (bool) {
        ...
        if (setPermTime(id, utime)) {
            ...
            setLog([sn, uaddrids[msg.sender], perm.itemid, id], "setPermTimeByIdx", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    function setPermStatusById(bytes32 sn, string details, bytes32 id, uint64 newstatus) public onlyValid returns (bool) {
        if (setPermStatus(id, newstatus)) {
            ...
            setLog([sn, uaddrids[msg.sender], perm.itemid, id], "setPermStatusById", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    function setPermStatusByIdx(bytes32 sn, string details, uint64 idx, uint64 newstatus) public onlyValid returns (bool) {
        ...
        if (setPermStatus(id, newstatus)) {
            ...
            setLog([sn, uaddrids[msg.sender], perm.itemid, id], "setPermStatusByIdx", 0x00, details, 0, uint64(block.timestamp));
            return true;
        } else {
            revert();
        }
    ...
    */
    ...
    //@return 0: 是否设置成功
    */
    function setLog(bytes32[4] ids, bytes32 action, bytes32 hash, string desc, uint64 duration, uint64 timestamp) internal returns (bool) {
        lidxids[lognum++] = ids[0];
        logs[ids[0]] = Log(ids[0], ids[1], ids[2], ids[3], action, hash, desc, duration, timestamp);
    ...
    //@return 0: 是否设置成功
    */
    function addLog(bytes32 sn, string details, bytes32[4] ids, bytes32 action, bytes32 lhash, string desc, uint64 duration, uint64 mode) public onlyAdmin returns (bool) {
        if (setLog(ids, action, lhash, desc, duration, uint64(block.timestamp))) {
            if (mode != 0) {
    ...
    */
    ...
    //@return 5: 日志生成时间戳
    */
    function getLogById(bytes32 id) public view returns (bytes32[4], bytes32, bytes32, string, uint64, uint64) {
        if (id == 0x00) {
            return ([bytes32(0x00), 0x00, 0x00, 0x00], bytes32(0x00), 0x00, "", uint64(0), 0);
        } else {
            Log memory log = logs[id];
            return ([log.id, log.userid, log.itemid, log.permid], log.action, log.lhash, log.desc, log.duration, log.timestamp);
        }
    }
    ...
    //@return 5: 日志生成时间戳
    */
    function getLogByIdx(uint64 idx) public view returns (bytes32[4], bytes32, bytes32, string, uint64, uint64) {
